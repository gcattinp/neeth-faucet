...
ifeq ($(findstring --network arbitrum,$(ARGS)),--network arbitrum)
	NETWORK_ARGS := --legacy --rpc-url $(ARBITRUM_RPC_URL) --private-key $(PRIVATE_KEY) --broadcast --verify --etherscan-api-key $(ARBISCAN_API_KEY) -vvvv
endif

deploy:
	@forge script script/DeployFaucet.s.sol:DeployFaucet $(NETWORK_ARGS)
